...
### Contract unit tests (`cargo test`, 7 passing)
...
| `happy` | Registry checked, escrow deployed, funded, stage 1 approved and released, stage 2 disputed and split 50/50, stage 3 released, case closed, outcome recorded in the registry |
...
- The server does not yet verify a Privy access token; the patient is identified by the plan's email and the wallet that accepted it.
